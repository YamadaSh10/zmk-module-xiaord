	...
	for (int i = 0; i < ZMK_SPLIT_CENTRAL_PERIPHERAL_COUNT; i++) {
		...
		if (state.valid[i]) {
			if (arc) {
				...
				if (state.level[i] > 50) {
					lv_obj_set_style_arc_color(arc, lv_color_hex(0x76FF03), LV_PART_INDICATOR); //light-green-accent-3
				} else if (state.level[i] > 20) {
					lv_obj_set_style_arc_color(arc, lv_color_hex(0xFFFF00), LV_PART_INDICATOR); //yellow-accent-2
				} else {
					lv_obj_set_style_arc_color(arc, lv_color_hex(0xD50000), LV_PART_INDICATOR); //red-accent-4
				}
			}
			if (lbl) {
